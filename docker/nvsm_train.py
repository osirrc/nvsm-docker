...
def main(_):
        ...
        for epoch in range(opts.epochs):
            ...
            for i in range(num_batches):
                ...
                if (i + 1) % save_embeddings_every == 0:
                    model_checkpoint_path = os.path.join(os.getcwd(), models_folder_name + '/' + opts.model_name, opts.model_name + str(epoch + 1) + '.ckpt')
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.import_meta_graph(os.path.join(models_folder_name, opts.model_name) + '/' + opts.model_name + str(best_epoch) + '.ckpt.meta')
        saver.restore(sess, os.path.join(models_folder_name, opts.model_name) + '/' + opts.model_name + str(best_epoch) + '.ckpt')
