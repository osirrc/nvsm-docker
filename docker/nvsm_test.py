...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.import_meta_graph(os.path.join(models_folder_name, opts.model_name) + '/' + opts.model_name + best_epoch + '.ckpt.meta')
        saver.restore(sess, os.path.join(models_folder_name, opts.model_name) + '/' + opts.model_name + best_epoch + '.ckpt')
        ...
        utils.perform_search(doc_labels, doc_embs, query_ids, queries, rankings_folder_name + '/' + opts.model_name, opts.model_name + 'test_best_epoch')
        utils.evaluate_rankings(rankings_folder_name + '/' + opts.model_name, opts.model_name + 'test_best_epoch', qrels_folder_name, 'test_qrels')
