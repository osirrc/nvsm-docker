...
class Utils(object):
    ...
    def build_term_dictionary(self, index, dict_size=65536, oov=False, remove_digits=True, min_doc_freq=2, max_doc_freq=0.5):
    ...
    def evaluate_rankings(self, ranking_folder, ranking_name, qrels_folder, qrels_name):
        ...
        MAP = self.get_averaged_measure_score(ranking_folder + '/' + ranking_name + '.txt', qrels_folder + '/' + qrels_name + '.qrel', 'map')
        NDCG = self.get_averaged_measure_score(ranking_folder + '/' + ranking_name + '.txt', qrels_folder + '/' + qrels_name + '.qrel', 'ndcg_cut_100')
        P_10 = self.get_averaged_measure_score(ranking_folder + '/' + ranking_name + '.txt', qrels_folder + '/' + qrels_name + '.qrel', 'P_10')
