...
def get_recursively_files_in_folder(root_folder):
    # uncompress .z files within root folder
    print("uncompress collection...")
    cmd = 'bash extract_robust.sh ' + root_folder
    os.system(cmd)
    print("collection uncompressed!")
    corpus_path = '/unzipped/'
    files = []
    exclude_dirs = {'hfiles', 'dtds', 'efiles', 'fr94', 'ft'}
    # r=root, d=directories, f = files
    for r, d, f in os.walk(corpus_path):
        d[:] = [dirs for dirs in d if dirs.lower() not in exclude_dirs]
        for file in f:
            if 'read' not in file \
                    and os.path.isfile(os.path.join(r, file)) \
...
def main():
    ...
    # remove unnecessary folder (i.e. unzipped)
