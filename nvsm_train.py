...
def main(_):
    ...
    print('%d queries found' % len(qdict.items()))
    # generate query subsets for validation and test
    print('split queries into validation and test subsets')
    # load validation ids
    with open(opts.splits_val_path, 'r', encoding='utf-8') as val_file:
        val_ids = val_file.read()
    val_ids = val_ids.split('\n')
    print('%d validation ids' % len(val_ids))
    # load test ids
    with open(opts.splits_test_path, 'r', encoding='utf-8') as test_file:
        test_ids = test_file.read()
    test_ids = test_ids.split('\n')
    print('%d test ids' % len(test_ids))
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for epoch in range(opts.epochs):
            ...
            print('qrels folder: ' + qrels_folder_name)
